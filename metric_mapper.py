...
def find_wind_mph(weather_path, date, county):
    ...
    for i in range(len(weather_csv)):
        ...
        if row["BEGIN_DATE"] == date and row["CZ_NAME_STR"].replace("(ZONE)", "").replace("CO.", "").strip().lower() == county.lower():
...
def combine_weather_data(weather_path, outage_record_path, save_path, year):
    county_metrics = pd.DataFrame(columns=["date", "county", "num_people_out", "wind_speed_mph"])
    ...
    for county, dates in eagle_I_data.items():
        for cur_date, events in dates.items():
            ...
            if wind_speed:
                row_df = pd.DataFrame([[cur_date, county, max_people_out, wind_speed]], 
                                    columns=["date", "county", "num_people_out", "wind_speed_mph"])
